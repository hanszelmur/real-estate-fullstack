/**
 * Agent Messages JavaScript
 * 
 * Handles messaging functionality for agents including:
...
async function loadMessages() {
    ...
    const endpoint = currentMessageTab === 'inbox' 
        ? `/messages/inbox?page=${currentMessagePage}` 
        : `/messages/sent?page=${currentMessagePage}`;
        ...
        if (messages.length === 0) {
            container.innerHTML = `
                <div class="text-center" style="padding: 40px;">
                    <p>${currentMessageTab === 'inbox' ? 'No messages in your inbox.' : 'No sent messages.'}</p>
                    ${currentMessageTab === 'inbox' ? '<p style="margin-top: 10px;">Messages from customers will appear here.</p>' : ''}
                </div>
            `;
        ...
        let html = `
            <div class="message-detail">
                ...
                <div class="message-meta">
                    <p><strong>From:</strong> ${escapeHtml(msg.sender_first_name)} ${escapeHtml(msg.sender_last_name)} (${capitalize(msg.sender_role)})</p>
                    <p><strong>To:</strong> ${escapeHtml(msg.recipient_first_name)} ${escapeHtml(msg.recipient_last_name)} (${capitalize(msg.recipient_role)})</p>
...
/**
 * Load message recipients (customers with appointments)
 */
async function loadMessageRecipients() {
    ...
    if (response.ok && response.data.success) {
        ...
        for (const apt of appointments) {
            if (apt.customer_id && apt.customer_first_name) {
                customerMap.set(apt.customer_id, {
                    id: apt.customer_id,
                    name: `${apt.customer_first_name} ${apt.customer_last_name}`,
        ...
        if (customerMap.size > 0) {
            select.innerHTML = '<option value="">-- Select recipient --</option>' +
                Array.from(customerMap.values()).map(customer => 
                    `<option value="${customer.id}">${escapeHtml(customer.name)} (Customer)</option>`
                ).join('');
        } else {
